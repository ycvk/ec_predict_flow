...
from app.v2.usecases.steps.label_calculation import calculate_labels_df
...
@celery_app.task(name="v2.label_calculation")
def label_calculation(
    *,
    run_id: str,
    step_id: str,
    raw_artifact_id: str,
    ...
    try:
        ...
        repo.set_step_status(step, StepStatus.RUNNING, progress=20, message="开始计算标签")
        session.commit()
        ...
        labels_df, stats = calculate_labels_df(
            raw_df=df,
            window=int(window),
        ...
        repo.set_step_status(step, StepStatus.RUNNING, progress=90, message="写入 labels 产物")
        session.commit()

        filename = f"labels_{label_type}_{filter_type}_w{int(window)}_f{int(look_forward)}.parquet"
        uri = artifacts.artifact_uri(run_id=run_id, kind=ArtifactKind.LABELS, filename=filename)
        ...
        labels_artifact = repo.add_artifact(
            run_id=run_id,
            step_id=step_id,
            kind=ArtifactKind.LABELS,
            uri=uri,
            sha256=sha256,
            bytes_=bytes_,
            metadata={
        ...
        is_pipeline = continue_pipeline_if_needed(
            ...
            run=run,
            step=step,
            produced_state_patch={"labels_artifact_id": labels_artifact.id},
        )
        if not is_pipeline:
        ...
        return {
            "status": "success",
            "artifact_id": labels_artifact.id,
            "rows": int(len(labels_df)),
            "stats": stats,
        }
